...
public:
	void run(void)
	{
		...
		m_r.instanciateModel(m_m, "res/lev/", "000.obj");
		/*{
			auto [b, n] = m_m.addBrush<Id, Transform, MVP, MV_normal, OpaqueRender, RT_instance>(1);
		...
		while (true) {
			...
			m_r.render(m_m, Camera{last_view, view, proj, static_cast<float>(far), static_cast<float>(near), glm::vec2(ratio, -1.0) * glm::vec2(std::tan(fov / 2.0))}, t);
			first_it = false;
		}
